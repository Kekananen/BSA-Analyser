...
public class BSA_Visualisation extends javax.swing.JFrame {
    ...
    public static void file_checker(File fn) {
            ...
            if (fn.getName().endsWith("fa") || (fn.getName().endsWith("vcf")) || (fn.getName().endsWith("gff")) || (fn.getName().endsWith("gff3")) || (fn.getName().endsWith("fasta"))) {
                ...
                /**
                 * Selecting the files needed with file chooser, and they are
    ...
    private void Mapped_Allele_FrequencyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Mapped_Allele_FrequencyActionPerformed
        ...
        ArrayList<String> par_wt_vcf = new ArrayList<>();
        ArrayList<String> par_mt_vcf= new ArrayList<>();
        HashMap<String,HashMap<Integer,Double>> MAF_wt_output = new HashMap<>();
        HashMap<String,HashMap<Integer,Double>> MAF_mt_output = new HashMap<>();
        ...
        if (parent_file_wt != null & child_files_wt != null & parent_file_mt != null & child_files_mt != null) {
            ...
            try {
                par_wt_vcf = vcf_lines(parent_file_wt);
            } catch (IOException ex) {
                ex.printStackTrace();
            }
            
            try {
                par_mt_vcf = vcf_lines(parent_file_mt);
            } catch (IOException ex) {
                ex.printStackTrace();
            }
//          mt_vcf =  bsa_a.services.VcfFilter.filterUnMapped(mt_vcf);
//          wt_vcf =  bsa_a.services.VcfFilter.filterUnMapped(wt_vcf);
//          par_mt_vcf =  bsa_a.services.VcfFilter.filterUnMapped(par_mt_vcf);
//          par_wt_vcf =  bsa_a.services.VcfFilter.filterUnMapped(par_wt_vcf);
            //System.out.println("initial Output "+wt_vcf);
          
          SlidingWindows SW = new SlidingWindows();
                   MAF_wt_output = SW.CalculateWindows(mt_vcf, 2000, 2000, "MAF Estimation");
                   MAF_mt_output = SW.CalculateWindows(wt_vcf, 20000, 20000, "MAF Estimation");
                   
                  System.out.println("Start");
                  System.out.println("the results"+MAF_wt_output+"\n");
...
//       JComboBox box = new JComboBox(Create_BSA_Graph.draft_bsa.keySet().toArray());
////        for(Create_BSA_Graph.draft_bsa.values()){
////            box.addItem(values);
////        }
    ...
    private void jMenuItem3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItem3ActionPerformed
         ...
         // TODO add your handling code here:
        //Guided analysis
        List<File> analysisFile = new ArrayList<File>();
       // File[] analysisFile;
        String[] choices = {"MAF Estimation", "Homozygosity mapping", "Allelic Distance"};
        String input = (String) JOptionPane.showInputDialog(null, "Please Choose now...",
                "The Mapping Method of this Analysis", JOptionPane.QUESTION_MESSAGE, null, // Use
                // default
                // icon
                choices, // Array of choices
                choices[1]); // Initial choice
        if ((input.equals(choices[0])) || (input.equals(choices[1])) || (input.equals(choices[2]))) {
            int user_option = JOptionPane.showConfirmDialog(null, "Do you wish to Upload a Fasta File for your Analysis Right Now");
            if (user_option == 0) {
                JFileChooser chooser = new JFileChooser();
                chooser.setAcceptAllFileFilterUsed(false);
                FileNameExtensionFilter filter = new FileNameExtensionFilter("fasta file", "fasta", "fa");
                chooser.addChoosableFileFilter(filter);
                chooser.setMultiSelectionEnabled(false);
                chooser.showOpenDialog(new JFrame());
                File files = chooser.getSelectedFile();
                if ((files.getName().endsWith("fasta")) || (files.getName().endsWith("fa"))) {
                    FileValidator.fasta_content_checker(files);
                    analysisFile.add(files);
                } else {
                    JOptionPane.showMessageDialog(null, "Please Select the right Fasta File format for this " +input+" Analysis");
                }
            } else {
                return;
            }
            if (input.equals(choices[0])) {
                user_option = JOptionPane.showConfirmDialog(null, "Do you wish to Upload a VCF File for your " +input+" Analysis Right Now " + " Please Upload at least 4 VCF file");
                if (user_option == 0) {
                    JFileChooser chooser = new JFileChooser();
                    chooser.setAcceptAllFileFilterUsed(false);
                    FileNameExtensionFilter filter = new FileNameExtensionFilter("vcf file", "vcf");
                    chooser.addChoosableFileFilter(filter);
                    ...
                    if (files.length > 3) {
                        ...
                        for (File fn : files) {
                            if (fn.getName().endsWith("vcf")) {
                                FileValidator.vcf_content_checker(fn.getAbsolutePath());
                            }
                        }
                                Reader.vcfsReader(files);
                               
                                analysisFile.addAll(Arrays.asList(files));
                            } else {
                                JOptionPane.showMessageDialog(null, "Please Upload a file with the correct VCF extension");
                            }
                        
                    } else {
                        JOptionPane.showMessageDialog(null, "Please Upload at least 4 VCF Files, \n Coz MAF Estimation requires 2 parent files and 2 or more child file");
                }
                ...
                user_option = JOptionPane.showConfirmDialog(null, "Do you wish to Upload a VCF File for your " +input+ " Analysis Right Now " + " Please Upload a Maximum 2 VCF file");
                if (!input.equals(choices[0])) {
                    JFileChooser chooser = new JFileChooser();
                    chooser.setAcceptAllFileFilterUsed(false);
                    FileNameExtensionFilter filter = new FileNameExtensionFilter("vcf file", "vcf");
                    ...
                    if (files.length  >= 2) {
                        ...
                        JOptionPane.showMessageDialog(null, "Please Upload at Maximum 2 VCF Files");
            ...
            user_option = JOptionPane.showConfirmDialog(null, "Do you wish to Upload an annotation file or gff File for your Analysis Right Now");
            if (user_option == 0) {
                JFileChooser chooser = new JFileChooser();
                chooser.setAcceptAllFileFilterUsed(false);
                ...
                chooser.showOpenDialog(new JFrame());
                File files = chooser.getSelectedFile();
                if ((files.getName().endsWith("gff")) || (files.getName().endsWith("gff3"))) {
                    FileValidator.gff_content_checker(files.getAbsolutePath());
                    analysisFile.add(files);
                } else {
                    int user = JOptionPane.showConfirmDialog(null, "Do you wish to continue this analysis without uploading an Annotation file");
            ...
            File[] files = new File[ analysisFile.size() ];
            analysisFile.toArray( files );
            set_text_area(files);
            JOptionPane.showConfirmDialog(null, "Click yes to start running this " +input+  " analysis");
